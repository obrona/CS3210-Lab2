...
#pragma omp parallel for shared(a, b, result) private(i, j, k)
    for (i = 0; i < size; i++)
        // simply swap the order of summation
        // no race proble,, as each thread calculate the result of a set of rows, and no thread set of rows intersect
        // i.e thread 0 find result for rows 0, 1, 2, thread 1 find result for rows 2, 3, 4
        for (k = 0; k < size; k++)
            for (j = 0; j < size; j++)
